...
function copyDirectory(src, dest, auth) {
  ...
  const appName = path.basename(process.cwd()); // Get the app name
  ...
  // Set the output filename and directory for the resolved EJS files
...
inquirer.prompt(prompts).then((promptResults) => {
  ...
  //processEjsTemplates(templatesPath, promptResults, 'srcTemplates');
...
// function processEjsTemplates(srcPath, promptResults, isYaml = false) {
...
//     if (entry.isDirectory()) {
//       // Recursively process subdirectories
//       processEjsTemplates(fullPath, promptResults, isYaml);
//     } else if (path.extname(entry.name).toLowerCase() === '.ejs') {
//       const template = fs.readFileSync(fullPath, 'utf8');
//       const renderedTemplate = ejs.render(template, { promptResults, appName });

//       // Determine the output extension and directory based on the input parameter
//       const outputExtension = isYaml ? '.yaml' : '.jsx';
//       const baseName = path.basename(entry.name, '.ejs');

//       // Check if it's the catalog-info file
//       if (isYaml && baseName === 'catalog-info') {
//         const targetPath = path.join(process.cwd(), `${baseName}${outputExtension}`);
//         fs.writeFileSync(targetPath, renderedTemplate);
//       } else {
//         const targetDir = isYaml ? path.join(process.cwd(), 'templates') : path.join(process.cwd(), 'src/components', baseName);
//         const targetPath = path.join(targetDir, `${baseName}${outputExtension}`);
...
//   const componentsPath = path.resolve(__dirname, 'templates/src/components');
//   processEjsTemplates(componentsPath, promptResults, false);

//   // Process the catalog-info.ejs file separately
//   const catalogInfoPath = path.resolve(__dirname, './templates/catalog-info.ejs');
//   if (fs.existsSync(catalogInfoPath)) {
//     processEjsTemplates(templatesPath, promptResults, true);
//   }
// });